...
private:
    ...
    bool insertInTree(TreeNode* parent, DBentry* newEntry);
    DBentry* findInTree(TreeNode* parent, string name);
    bool removeFromTree(TreeNode*& parent, string name);
    TreeNode* findLargest(TreeNode* parent);
    void removeAll(TreeNode* parent);
    int activeInTree(TreeNode* curNode) const;
